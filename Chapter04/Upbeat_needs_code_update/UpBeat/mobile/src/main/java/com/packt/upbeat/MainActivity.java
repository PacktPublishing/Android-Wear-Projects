...
public class MainActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(StepCounts student : results){
//            text.append(student.getRoll_no() + " " + student.getName() + "\n");
            student.getReceivedDateTime();
            Log.d("time", student.getReceivedDateTime());
        }

        // Register the local broadcast receiver
        IntentFilter messageFilter = new IntentFilter(Intent.ACTION_SEND);
        HeartRateReciver messageReceiver = new HeartRateReciver();
...
//            new SendToDataLayerThread("/message_path","You:-\n" + message).start();
    ...
    public class HeartRateReciver extends BroadcastReceiver {
        @Override
        public void onReceive(Context context, Intent intent) {
            String message = intent.getStringExtra("heart");
            Log.v("myTag", "Main activity received message: " + message);

            mHeart.setText(message);
            heartbeat.setDurationBasedOnBPM(Integer.valueOf(message));
            heartbeat.toggle();
